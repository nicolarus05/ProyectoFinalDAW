...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .form-control {
            width:100%;padding:9px 14px;border:1.5px solid #e5e7eb;border-radius:9px;
            font-size:13px;outline:none;transition:border .2s;background:#fff;color:#1f2937;
        }
...
<div class="main-wrapper">
    <header class="topbar">
        ...
        <span class="page-title">✏️ Editar Cita</span>
        <div style="flex:1"></div>
        <a href="{{ route('citas.show', $cita->id) }}" style="font-size:12px;color:#a855f7;font-weight:600;text-decoration:none;margin-right:12px">← Ver detalle</a>
    ...
    <div class="content">
        <div class="panel">
            <div class="panel-header">
                <h2>✏️ Editar Cita #{{ $cita->id }}</h2>
                <a href="{{ route('citas.index') }}" style="font-size:11px;color:rgba(255,255,255,.7);text-decoration:none">← Volver al calendario</a>
            </div>
            <div class="panel-body">
                ...
                <form action="{{ route('citas.update', $cita->id) }}" method="POST">
                    ...
                    <div class="form-group">
                        <label class="form-label">📅 Fecha y Hora <span style="color:#ef4444">*</span></label>
                        <input type="datetime-local" name="fecha_hora" class="form-control"
                               value="{{ \Carbon\Carbon::parse($cita->fecha_hora)->format('Y-m-d\TH:i') }}" required>
                    </div>
                    ...
                    @else
                        ...
                        <div class="form-group">
                            ...
                            <div style="padding:9px 14px;background:#f9fafb;border:1.5px solid #e5e7eb;border-radius:9px;font-size:13px;color:#374151;font-weight:600">
                    ...
                    <div class="form-group">
                        <label class="form-label">👔 Empleado <span style="color:#ef4444">*</span></label>
                    ...
                    <div class="form-group">
                        <label class="form-label">✂️ Servicios <span style="color:#ef4444">*</span></label>
                        <div style="position:relative;margin-bottom:6px">
                            <span style="position:absolute;left:11px;top:50%;transform:translateY(-50%);font-size:13px;color:#9ca3af;pointer-events:none">🔍</span>
                            <input type="text" id="buscador-servicios" placeholder="Buscar servicio…"
                                   oninput="filtrarServicios(this.value)"
                                   style="width:100%;padding:8px 12px 8px 32px;border:1.5px solid #e5e7eb;border-radius:9px;font-size:13px;outline:none;transition:border .2s;background:#fafafa"
                                   onfocus="this.style.borderColor='#a855f7';this.style.boxShadow='0 0 0 3px rgba(168,85,247,.08)'"
                                   onblur="this.style.borderColor='#e5e7eb';this.style.boxShadow='none'">
                        </div>
                        <select name="servicios[]" id="select-servicios" multiple class="form-control" required style="min-height:140px">
                        ...
                        <p style="font-size:11px;color:#9ca3af;margin-top:4px">Mantén <kbd style="background:#f3f4f6;padding:1px 5px;border-radius:4px;font-size:10px">Ctrl</kbd> (Windows) o <kbd style="background:#f3f4f6;padding:1px 5px;border-radius:4px;font-size:10px">Cmd</kbd> (Mac) para seleccionar varios.</p>
                    </div>

                    <div class="form-group">
                        <label class="form-label">📝 Notas Adicionales</label>
                        <textarea name="notas_adicionales" rows="3" class="form-control" style="resize:vertical"
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;padding-top:16px;border-top:1px solid #f3f4f6">
                        <a href="{{ route('citas.index') }}" class="btn-secondary">← Volver</a>
...
function filtrarServicios(texto) {
    ...
    const opciones = document.querySelectorAll('#select-servicios option');
    opciones.forEach(opt => {
        const coincide = !q || opt.dataset.nombre.includes(q);
        opt.style.display = coincide ? '' : 'none';
        // Si el filtro quita una opción seleccionada, la mantenemos visible
        if (opt.selected && !coincide) opt.style.display = '';
    });
}
</script>
</body>
